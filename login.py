...
def login(self,socketio,enableCmdQR=False, picDir=None, qrCallback=None,
    ...
    while self.isLogging:
        ...
        if uuid:
            ...
            # buffer = BytesIO(qrStorage.getvalue())
            # buffer2 = BytesIO()
            # image = Image.open(buffer)
            # image.save(buffer2, format="JPEG")
            img_str = base64.b64encode(qrStorage.getvalue())
            count = 0
            socketio.emit('qrinfo', bytes.decode(img_str), namespace='/login')
        isLoggedIn = False
        while not isLoggedIn:
            ...
            if status == '200':
                ...
                if isLoggedIn is not None:
                    ...
                    socketio.emit('confirminfo', "confirm", namespace='/login')
                    isLoggedIn = None
            elif status != '408':
                break
        if isLoggedIn:
    ...
    socketio.emit('logining', "logining", namespace='/login')
    self.web_init()
...
def start_receiving(self,socketio,exitCallback=None, getReceivingFnOnly=False):
    self.alive = True
    def maintain_loop():
        retryCount = 0
        while self.alive:
...
def logout(self,socketio):
    socketio.emit('logout', 'logout', namespace='/test')
    if self.alive:
